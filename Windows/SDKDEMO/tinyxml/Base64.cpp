...
void CBase64::SetLineMaxLen(int lineMaxLen)
{
	m_lineMaxLen = lineMaxLen;
}

int CBase64::Encode(const unsigned char *pSrc, int srcLen, char *pDest, int destLen)
{
	int i = 0;
	int loop = 0;
	int remain = 0;
...
int CBase64::Decode(const char *pSrc, int srcLen, unsigned char *pDest, int destLen)
{
	int i = 0;
	int iCnt = 0;
	...
	for (i=0; i<srcLen; i++)
	{
		...
		if (pSrc[i] == '=')
		{
			// CBase64类在某些测试用例下不可逆，比如64个0x00字符，Encode过程是对的，Decode过程会多出一个字符 20180130 wzf
			return p-pDest+1;
		}
